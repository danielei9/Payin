/* global cordova */
/*
com.payin.palago.smartband
var exec = require('cordova/exec');

var NfcPlugin = function() { 
 console.log("NfcPlugin.js: is created");
}

NfcPlugin.prototype.load = function(aString){
 console.log("NfcPlugin.js: load");
...
var seNfc = {
    secureElement: function (callback, win, fail) {
        document.addEventListener("action", callback, false);
        cordova.exec(win, fail, "NfcAdapterSE", "action", []);
    }
}
...
var nfc = {
    ...
    execute: function(card, keys, script, operation, slot, success, error){
        cordova.exec(success, error, "NfcPlugin", "execute", [card, keys, script, operation, slot]);  
    },
    
    getNfcStatus: function(success, error){
        cordova.exec(success, error, "NfcPlugin", "getNfcStatus", []);  
    },

    write: function (callback, res, win, fail) {
        document.removeEventListener("ndef-formatable", callback);
        document.addEventListener("ndef-formatable", callback, false);
        cordova.exec(win, fail, "NfcPlugin", "write", [res]);
    },
    
    addCallback: function(getdetect, success, error) {
        if (getdetect) {
            document.removeEventListener("nfc-script-detect", getdetect, false);
            document.addEventListener("nfc-script-detect", getdetect, false);
        }
        if (success) {
            document.removeEventListener("nfc-script-success", success, false);
            document.addEventListener("nfc-script-success", success, false);
        }
        if (error) {
            document.removeEventListener("nfc-script-error", error, false);
            document.addEventListener("nfc-script-error", error, false);
        }
    },
    
    removeCallback: function(getdetect, success, error) {
        if (getdetect)
            document.removeEventListener("nfc-script-detect", getdetect);
        if (success)
            document.removeEventListener("nfc-script-success", success);
        if (error)
            document.removeEventListener("nfc-script-error", error);
    },
    
    getAllVC: function (win, fail) {
        cordova.exec(win, fail, "NfcPlugin", "getAllVC", []);
    },
    
    createVC: function (res, win, fail) {
        cordova.exec(win, fail, "NfcPlugin", "createVC", [res]);
    },
    
    deleteVC: function (res, win, fail) {
        cordova.exec(win, fail, "NfcPlugin", "deleteVC", [res]);
    },
    
    activateVC: function (res, win, fail) {
        cordova.exec(win, fail, "NfcPlugin", "activateVC", [res]);
    },
