...
public class Parsers {
	...
	public static int getVcStatus(byte[] recvData) {
		String TAG = "getVcEntry";
		
		if (recvData[0] == (byte) 0x4E) {
			Log.i(TAG, " Get VC status Failed ");
			return 0;
		} else if (recvData[0] == (byte) 0x9F && recvData[1] == (byte) 0x70) {
			int vcStatus = recvData[3];
			return vcStatus;
		} else {
			return 0;
		}
	...
	public static VirtualCard getVcEntry(int vctype, byte[] recvData) {
		String TAG = "getVcEntry";
		...
		if (recvData[0] == (byte) 0x4E) {
			...
			String vcEntryUid = bytArrayToHex(extract(recvData, 6, recvData[5]));

			return new VirtualCard(vctype, vcEntryInt, "", vcEntryUid, 0, "");
		} else {
			return null;
		}
	}
	...
	public static ArrayList<Integer> getVCList(byte[] recvData, Context ctx) {
		...
		if (recvData[0] == (byte) 0x4E) {
			...
			String[] vcListEntry = vcList.split("611A");

			for(int i = 1; i < vcListEntry.length; i++) {
				//int vcEntryId = Integer.parseInt(vcListEntry[i].substring(4, 8));
				int vcEntryId = byteArrayToInt(hexToArray(vcListEntry[i].substring(4, 8)));
	...
	public static short getSW(byte[] rapdu) {
		if(rapdu.length < 4)
			return StatusBytes.SW_SELECT_LTSM_FAILED;
		else {
