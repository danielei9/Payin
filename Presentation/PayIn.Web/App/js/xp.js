'use strict';
angular.module('xp', ['app', 'xp.communication', 'textAngular', 'xp.authentication', 'ngImgCrop', 'ngSanitize', 'ui.select', 'ngDialog', 'xpMap', 'xpChart', 'xpGraph', 'xpRedsys'])
	...
	.filter('xpNumber', ['$filter', function ($filter) {
		return function (input) {
			if (input % 1 === 0) {
	...
	.directive('xpGet', ['xpGet', function (xpGet) {
		// <xp-get api="api/sensor" no-initial-search xp-init=scope.id = params.id>
		return {
			restrict: 'E',
			priority: 100,
			scope: true,
	...
	.directive('xpDuration', [function () {
		...
		return {
            ...
            controller: ['$scope', '$attrs', '$element', function ($scope, $attrs, $element) {
                var format = $attrs["xpFormat"] || 'hh:ii';
                var step = parseInt($attrs["xpStep"] || 5);
                var startview = parseInt($attrs["xpStartview"] || 1);
                var minview = parseInt($attrs["xpMinview"] || 1);
                var maxview = parseInt($attrs["xpMaxview"] || 1);
