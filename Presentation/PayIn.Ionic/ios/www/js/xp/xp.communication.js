angular.module('xp.communication', [])
	...
	.factory('xpInitialScope', [function () {
		return {
			'initialize': function () {
				//var that = this;
				//this.$on('clearInbound', function (event) {
				//	that.id = "";
				//	that.arguments = {};
				//	that.autofile = {};
				//	that.files = {};
				//	that.temp = {};
				//});
				//this.$on('clearOutbound', function (event) {
				//	that.data = []
				//	that.count = 0;
				//	that.currentPage = 1;
				//	that.errors = [];
			...
			'connect': function () {
				this.errors = [];
				if ((this.windowState().isBusy === 0) && this.show)
					this.show();
			...
			'success': function (result) {
				...
				if ((that.windowState().isBusy === 0) && that.hide)
					that.hide();
			...
			'error': function (result) {
				...
				var message =
					result.error_description || // Login error
