...
angular.module('xp', ['xp.communication', 'xp.authentication'])
	...
	.factory('xpPost', ['xpCommunication', 'xpStack', '$interpolate', '$ionicHistory', '$q',
		function (xpCommunication, xpStack, $interpolate, $ionicHistory, $q) {
			return {
				'accept': function () {
					var deferred = $q.defer();
					...
					xpCommunication
						...
						.post(that.apiUrl, "", params, null, that.files)
						.success(that.success.bind(that), function (data) {
							if (that.onSuccess) {
								var action = new Function('scope', 'xpStack', 'data', 'navigation', that.onSuccess);
								action(that, xpStack, data, $ionicHistory);
							}
