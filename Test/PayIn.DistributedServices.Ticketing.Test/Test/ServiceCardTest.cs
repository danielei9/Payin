...
namespace PayIn.DistributedServices.Ticketing.Test
{
	...
	{
		[TestMethod]
		public async Task ServiceCardManagementAsync()
		{
			using (var Server = new TicketingBusinessServer())
			{
				...
				// Get SystemCard
				var systemCard = (await Server.SystemCardGetSelectorAsync())
					.FirstOrDefault();
				Assert.IsNotNull(systemCard);

				// Create ServiceUser
				var serviceUser = await Server.ServiceUserCreateAsync(systemCard.Id);
