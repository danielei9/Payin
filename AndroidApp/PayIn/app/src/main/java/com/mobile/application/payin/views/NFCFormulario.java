...
public class NFCFormulario extends AppCompatActivity implements AsyncResponse {
    ...
    private static HashMap<Integer, Pair<ServiceTagGetResult.Value, ControlPresenceCheckArguments.Value>> FormValues = new HashMap<>();
    private static Pair<ServiceTagGetResult.Value, ControlPresenceCheckArguments.Value> pairImage;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Forms = (ArrayList<ServiceTagGetResult.Item>) getIntent().getSerializableExtra("forms");
    ...
    private void createForms(){
        ...
        ControlPresenceCheckArguments.Assign argAssing;
        ControlPresenceCheckArguments.Value argValue;
        ...
        for (ServiceTagGetResult.Item item : Forms) {
            ...
            for (ServiceTagGetResult.Assign resAssign : plan.Assigns) {
                argAssing = new ControlPresenceCheckArguments.Assign();
                ...
                for (ServiceTagGetResult.Value resValue : resAssign.Values) {
                    ...
                    if (resValue.Target == FormValueTargetType.CREATION) {
                        ...
                        argValue = new ControlPresenceCheckArguments.Value();
                ...
                argItem.Assigns.add(argAssing);
                scrollForms.addView(childLayout);
            }
        }
    }

    private void createStringTxt(ServiceTagGetResult.Value resValue, ControlPresenceCheckArguments.Value argValue, LinearLayout ll) {
    ...
    private void createNumericTxt(ServiceTagGetResult.Value resValue, ControlPresenceCheckArguments.Value argValue, LinearLayout ll) {
        EditText et = new EditText(context);
    ...
    private void createDateTimeBtn(ServiceTagGetResult.Value resValue, ControlPresenceCheckArguments.Value argValue, LinearLayout ll) {
    ...
    private void createDateBtn(ServiceTagGetResult.Value resValue, ControlPresenceCheckArguments.Value argValue, LinearLayout ll) {
        Button btn = new Button(context);
    ...
    private void createTimeBtn(ServiceTagGetResult.Value resValue, ControlPresenceCheckArguments.Value argValue, LinearLayout ll) {
        Button btn = new Button(context);
    ...
    private void createDurationBtn(ServiceTagGetResult.Value resValue, ControlPresenceCheckArguments.Value argValue, LinearLayout ll) {
        Button btn = new Button(context);

        btn.setId(countFormValues);

        if (resValue.ValueDateTime == null)
            btn.setText("Seleccionar una duración");
        else {
            TimeZone tz = TimeZone.getTimeZone("UTC");
            SimpleDateFormat sdfUTC = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss", Locale.getDefault()),
                    sdfLocal = new SimpleDateFormat("HH:mm:ss", Locale.getDefault());
    ...
    private void createCheckBox(ServiceTagGetResult.Value resValue, ControlPresenceCheckArguments.Value argValue, LinearLayout ll) {
    ...
    private void createImgBtn(ServiceTagGetResult.Value resValue, ControlPresenceCheckArguments.Value argValue, LinearLayout ll) {
        Button btn = new Button(context);
    ...
    private void dispatchTakePictureIntent(Pair<ServiceTagGetResult.Value, ControlPresenceCheckArguments.Value> values) {
    ...
    private void fichar() {
        ...
        for (Pair<ServiceTagGetResult.Value, ControlPresenceCheckArguments.Value> pair : FormValues.values()){
            if (pair.res.IsRequired){
                ControlPresenceCheckArguments.Value argValue = pair.arg;
        ...
        if (falta){
            ...
            if (saveFacial) {
                Intent i = new Intent(NFCFormulario.this, NFCFoto.class);
    ...
    public static class TimePickerFragment extends DialogFragment implements TimePickerDialog.OnTimeSetListener {
        ...
        public void onTimeSet(TimePicker view, int dialogHour, int dialogMinute) {
            ...
            String fechaLocal = String.format("%02d:%02d:00", dialogHour, dialogMinute),
                    fechaServer;
