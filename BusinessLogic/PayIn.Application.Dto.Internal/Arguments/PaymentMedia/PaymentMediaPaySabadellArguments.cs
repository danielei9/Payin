...
{
	public class PaymentMediaPaySabadellArguments : IArgumentsBase
	{
		...
		#region Constructors
		public PaymentMediaPaySabadellArguments(string ds_Date, string ds_Hour, int ds_Amount, int ds_Currency, int ds_Order, int ds_MerchantCode, int ds_Terminal, string ds_Signature, int ds_Response, string ds_TransactionType, bool ds_SecurePayment, string ds_MerchantData, int ds_Card_Country, string ds_AuthorisationCode, int ds_ConsumerLanguage, string ds_Card_Type)
		{
			Ds_Date = ds_Date;
			Ds_Hour = ds_Hour;
			Ds_Amount = ds_Amount;
