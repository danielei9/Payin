...
{
	public class JustMoneyMobilePrepaidCardDisableHandler : IServiceBaseHandler<JustMoneyMobilePrepaidCardDisableArguments>
	{
		...
		[Dependency] public JustMoneyApiPrepaidCardEnableDisableHandler JustMoneyApiPrepaidCardEnableDisableHandler { get; set; }
		[Dependency] public PfsService PfsService { get; set; }

		#region ExecuteAsync
		public async Task<dynamic> ExecuteAsync(JustMoneyMobilePrepaidCardDisableArguments arguments)
		{
			...
			if (
				...
				throw new ApplicationException("Solo se pueden desactivar tarjetas activas");
			
			// Change state
			var result = await PfsService.ChangeCardStatusAsync(item.CardHolderId, cardInquiry.CardInfo.CardStatus, CardStatus.DepositOnly);
