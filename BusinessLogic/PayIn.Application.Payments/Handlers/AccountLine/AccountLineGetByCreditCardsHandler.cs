...
{
    public class AccountLineGetByCreditCardsHandler :
		IQueryBaseHandler<AccountLineGetByCreditCardsArguments, AccountLineGetByCreditCardsResult>
	{
		[Dependency] public ISessionData SessionData { get; set; }
        [Dependency] public IEntityRepository<AccountLine> Repository { get; set; }

		#region ExecuteAsync
		public async Task<ResultBase<AccountLineGetByCreditCardsResult>> ExecuteAsync(AccountLineGetByCreditCardsArguments arguments)
		{
            ...
            var items = (await Repository.GetAsync())
				.Where(x =>
					...
					(x.Ticket.Date >= since) &&
					(x.Ticket.Date < until) &&
                    (x.Type == AccountLineType.CreditCard) &&
                ...
                .Select(x => new
                {
				    ...
				    x.Uid,
                    PaymentMedia = x.Ticket.Payments
                        .Select(y => y.PaymentMedia.NumberHash)
                        .FirstOrDefault(),
            ...
            var result = items
                ...
                .ToList()
                .Select(x => new AccountLineGetByCreditCardsResult
                {
                    Id = x.Id,
            ...
            return new AccountLineGetByCreditCardsResultBase
            {
                Data = result,
