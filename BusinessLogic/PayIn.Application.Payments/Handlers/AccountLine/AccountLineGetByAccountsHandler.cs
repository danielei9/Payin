...
{
	public class AccountLineGetByAccountsHandler :
		IQueryBaseHandler<AccountLineGetByAccountsArguments, AccountLineGetByAccountsResult>
	{
		...
		#region ExecuteAsync
		public async Task<ResultBase<AccountLineGetByAccountsResult>> ExecuteAsync(AccountLineGetByAccountsArguments arguments)
		{
			...
			var result = items
                ...
                .ToList()
                .Select(x => new AccountLineGetByAccountsResult
                {
                    Id = x.Id,
                    ...
                    AccountTypeName = x.AccountType.ToEnumAlias()
                })
                .ToList();

			var sumPerConcession = result
				.Where(x =>
					(x.ConcessionId != null) &&
					!x.Paid &&
					!x.Liquidated &&
					(
						x.AccountType == AccountLineType.Products ||
						x.AccountType == AccountLineType.Entrances
					)
				)
				.GroupBy(x => new { x.ConcessionId, x.ConcessionName })
				.Select(x => new AccountLineGetByAccountsResult_SumPerConcession
				{
					Id = x.Key.ConcessionId.Value,
					Name = x.Key.ConcessionName,
					Amount = x.Sum(y => (decimal?)y.Amount) ?? 0
				})
				.ToList();

			return new AccountLineGetByAccountsResultBase
            {
                Data = result,
				SumPerConcession = sumPerConcession,
