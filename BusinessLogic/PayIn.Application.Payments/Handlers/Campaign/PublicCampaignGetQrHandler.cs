...
{
    public class PublicCampaignGetQrHandler : IQueryBaseHandler<PublicCampaignGetQrArguments, PublicCampaignGetQrResult>
	{
		[Dependency] public IEntityRepository<Campaign> Repository { get; set; }
		[Dependency] public ISessionData SessionData { get; set; }

		#region ExecuteAsync
		public async Task<ResultBase<PublicCampaignGetQrResult>> ExecuteAsync(PublicCampaignGetQrArguments arguments)
        {
            ...
            var resultBase = new ResultBase<PublicCampaignGetQrResult>
            {
                Data = result
                    .Select(x => new PublicCampaignGetQrResult
                    {
                        Id = x.Id,
                        Title = x.Title,
                        Since = x.Since,
                        Until = x.Until,
                        Caducity = x.Caducity,
                        Qr = PublicCampaignGetByUserHandler.GenerateQr(x.Qr)
                    })
            };
            return resultBase;
        }
