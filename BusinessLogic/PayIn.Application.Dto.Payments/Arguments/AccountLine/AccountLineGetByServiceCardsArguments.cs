...
{
    public class AccountLineGetByServiceCardsArguments : IArgumentsBase
	{
		...
		#region Constructors
		public AccountLineGetByServiceCardsArguments(string filter, XpDate since, XpDate until, int? serviceCardId, int? eventId, int? serviceConcessionId, TicketLineType? type)
		{
			Filter = filter ?? "";
			Since = since;
			Until = until;
