...
{
	[XpLog("TransportOperation", "Emite", RelatedId = "OperationId")]
	[XpAnalytics("TransportOperation", "Emite")]
	public class TransportOperationEmiteHandler : TransportOperationInternal,
		IServiceBaseHandler<TransportOperationEmiteArguments>
	{
		...
		#region Constructors
		public TransportOperationEmiteHandler(
		...
		#region ExecuteAsync
		public async Task<dynamic> ExecuteAsync(TransportOperationEmiteArguments arguments)
		{
#if TEST || DEBUG
			try
			{
				...
				var item = new ScriptResult
				{
					...
					Script = resultScript,
					Keys = await script.GetKeysEncryptedAsync(Hsm, CardSystem.Mobilis, resultScript, script.Card.Uid.ToInt64() ?? 0, script.Card.TituloTuiN.VersionClaves.Value)
				};
