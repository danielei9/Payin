...
{
	public class ApiSensorGetEnergyHandler :
        IQueryBaseHandler<ApiSensorGetEnergyArguments, ApiSensorGetEnergyResult>
	{
		...
		#region ExecuteAsync
		public async Task<ResultBase<ApiSensorGetEnergyResult>> ExecuteAsync(ApiSensorGetEnergyArguments arguments)
		{
			decimal? max = null;

			var endDateTime =
				(arguments.Period == ApiSensorGetEnergyArguments_Period.Day) ?
			...
			var sensor = (await Repository.GetAsync())
				...
				.FirstOrDefault();
			if (sensor == null)
				return new ApiSensorGetEnergyResultBase();
			...
			var result = result2
				.Select(x => new ApiSensorGetEnergyResult
				{
					Label = x.Label,
					// P1
